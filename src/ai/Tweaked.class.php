...
class Tweaked extends ConnectFourStrategy
{
    ...
    private function _calcScore($column, ConnectFourGame $game) {
        return $this->_calcHorizontalValue($column, $game, $this->player) + 
                $this->_calcVerticalValue($column, $game, $this->player) * (1 + (rand(0,100)/100)) +
                $this->_calcLeftDiagnalValue($column, $game, $this->player) +
                $this->_calcRightDiagnalValue($column, $game, $this->player) +
                $this->_calcHorizontalValue($column, $game, $this->opponent) * 1.5  + 
                $this->_calcVerticalValue($column, $game, $this->opponent)  * (1 + (rand(0,100)/100)) * 1.5 +
                $this->_calcLeftDiagnalValue($column, $game, $this->opponent) * 1.5 +
                $this->_calcRightDiagnalValue($column, $game, $this->opponent) * 1.5      
            ;
    }
